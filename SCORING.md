...
## Phân tích 1 câu grading (10 điểm)
...
- **Baseline (dense):**
  - Faithfulness: **1/5**
  - Relevance: **3/5**
  - → Model **hallucinate**, tự suy đoán lỗi dù không có context

- **Variant (hybrid + rerank):**
  - Faithfulness: **5/5**
  - Relevance: **5/5**
  - → Model trả lời đúng và grounded hơn

---

### Failure Mode

> **Generation + Retrieval failure (Baseline)**

- Model **bịa thông tin (hallucination)** về lỗi ERR-403-AUTH  
- Không phát hiện rằng **context không chứa thông tin liên quan**

---

### Root Cause

**1. Retrieval issue (secondary):**
- Dense retrieval không match được keyword “ERR-403-AUTH”
- Không retrieve được chunk liên quan → context rỗng / không đủ

**2. Generation issue (primary):**
- Prompt chưa ép model:
  - “Nếu không có thông tin → phải trả lời không biết”
- Model fallback sang **prior knowledge → hallucinate**

---

### Tại sao Variant fix được?

- **Hybrid retrieval**:
  - Match tốt hơn với keyword dạng code (`ERR-403`)
- **Rerank**:
  - Chọn chunk phù hợp hơn hoặc xác định không có context
- → Model không còn bị “context thiếu nhưng vẫn trả lời”

---

### Đề xuất cải thiện

**1. Prompt Fix (Generation):**
